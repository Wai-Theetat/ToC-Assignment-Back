...
def masked_text(input):
    """
    masked_text ทำงานอย่างไร

    โจทย์กำหนดให้นำค่า input ที่มีข้อมูล บัตรเครดิต อีเมล เบอร์โทรศัพท์ วันเกิด และที่อยู่ 
    โดยที่มีข้อมูลจะไม่ได้เรียงตามที่กล่าวข้างต้น โดยจะมีอะไรขึ้นก่อนก็ได้ และไม่ใส่บางตัวก็ได้
    
    หน้าที่ของ function นี้จะทำการเซ็นเซอร์ข้อมูลที่ไม่สามารถรู้ได้นี้
    
    อ่านต่อที่ find_details(input)
...
def find_details(input):
    ...
    บัตรเครดิต จะมาในรูปแบบ (ตัวเลข 4 ตัว - ตัวเลข 4 ตัว - ตัวเลข 4 ตัว - ตัวเลข 4 ตัว) สามารถดักได้

    เบอร์โทรศัพท์ จะมาในรูปแบบ (ตัวเลข 3 ตัว - ตัวเลข 3 ตัว - ตัวเลข 4 ตัว) สามารถดักได้

    อีเมล (คาดว่าผู้อ่านน่าจะรู้จักรูปแบบอยู่แล้ว) สามารถดักได้

    วันเกิด จะมาในรูปแบบ : วัน(ตัวเลข)/เดือน(ตัวเลข)/ปี(ตัวเลข) สามารถดักได้

    แต่ปัญหาคือที่อยู่ ที่ไม่สามารถดักได้เนื่องจากมีรูปแบบไม่ตายตัว ดังนั้นกลุ่มของพวกเรา จึงทำการใช้ส่วนที่รู้มาหาส่วนที่ไม่รู้
    เรารู้ว่าส่วนไหนคือบัตรเครดิต เบอร์โทรศัพท์ อีเมล วันเกิด เราจะสร้าง string ตัวหนึ่งชื่อ temp เป็น buffer จากนั้นเราจะลบส่วนที่เรารู้ออกจาก temp
    ก็จะเหลือส่วนที่ไม่ถูกลบก็คือ address ก็จะได้ข้อมูลทั้งหมดแล้ว 
